...
def evaluation(grid, n_empty): #grid 보드, n_empty 빈 보드
    grid = np.array(grid)
...
def maximize(grid, depth=0): #재귀로 보드, 깊이
    best_score = -np.inf
    best_action = None

    for action in range(4): # 4방향으로 움직였을때 값을 확인
        ...
        new_score = add_new_tiles(moved_grid, depth + 1) # 새로운 타일 랜덤으로 입력
        if new_score >= best_score: # 넣었을때 이전보다 점수가 높으면 갱신
...
def add_new_tiles(grid, depth=0):
    ...
    # early stopping
    if n_empty >= 10 and depth >= 3: # 빈칸이 6칸 이상이면 깊이 3가지
        return evaluation(grid, n_empty)

    if n_empty >= 0 and depth >= 5: # 빈칸이 좀더 더 딮하게 계산
        return evaluation(grid, n_empty)
    ...
    for x, y in fcs: #free cell 위치에 2,4 를 넣음
        for v in [2, 4]:
            ...
            if v == 2:
                new_score *= (0.9 / n_empty) # 2는 90% 확률이므로 0.9
            else:
                new_score *= (0.1 / n_empty) # 4는 10% 확률이므로 0.1
